...
| 12 | `ci: add the continuous integration workflow` | `.github/workflows/ci.yml`: `actions/checkout@v5`, `astral-sh/setup-uv@v5`, then `uv sync --locked`, `ruff check`, `ruff format --check`, `pyrefly check`, `pytest` with postgres + redis services | Push → green badge. Break the lint on purpose → red badge, then fix. Verify that a `pyproject.toml` / `uv.lock` mismatch makes `--frozen` **fail** |
